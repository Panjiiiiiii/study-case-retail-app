...
import { clearCart, removeFromCart, updateCartItemQuantity } from "@/lib/cart";
...
export default function CartList({ onClose }) {
  ...
  const handlePayment = () => {
    ...
    // Implementasi payment logic di sini
    toast.success('Proses pembayaran...');
  };
    ...
    <aside className="w-[400px] bg-[#E5E7EB] flex flex-col justify-between h-screen shadow-lg">
          ...
          <div className="space-y-4">
            {cartItems.length === 0 ? (
              <div className="flex flex-col items-center py-8">
                <img src="/trolley.png" alt="Empty Cart" className="w-24 h-24 mb-4" />
      ...
      <div className="p-8 border-t border-gray-300">
        <div className="flex justify-between items-center py-4">
          <P className="font-medium text-sky-950">Total ({totalQuantity} items)</P>
          <P className="font-bold text-lg text-sky-950">{formatPrice(total)}</P>
        </div>
        <div className="flex flex-row justify-between gap-4 mt-4">
          <Button 
            variant="default" 
            className="flex flex-row items-center gap-2 px-4 py-2 rounded-4xl text-lg font-medium"
